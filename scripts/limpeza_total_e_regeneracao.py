...
# -*- coding: utf-8 -*-
"""
Limpeza Total e Regeneração de TODAS as queries não-portuguesas.
Estratégia:
1. DELETAR TODAS as entradas corruptas/mal traduzidas
2. REGENERAR do zero com traduções LIMPAS e CORRETAS apenas
"""
import sqlite3

DB_PATH = "falhas_mercado_v1.db"

# 50 títulos em português - a fonte de verdade
TITULOS_PT = [
    'Ausência de Políticas Claras sobre Governança e Soberania de Dados',
    'Ausência de Políticas Imigratórias Eficazes de Incentivo à Inovação',
    'Ausência de Regulamentação sobre "Stock Options"',
    'Aversão ao Risco e Empreendedorismo de Necessidade',
    'Baixa Representatividade de Gênero e Raça',
    'Baixo Nível de Confiança Para Geração de Negócios',
    'Baixo Nível de Internacionalização',
    'Baixo Nível de Transferência de Tecnologia Entre Universidades e Empresas',
    'Barreiras ao Capital Estrangeiro',
    'Barreiras de Entrada e Compras Públicas',
    'Bases Curriculares Nacionais Desatualizadas',
    'Burocracia e lentidão excessivas no processo de abertura de empresas',
    'Concentração Extrema do Venture Capital (VC) em Geografia, Setores e Fases de Maturidade das Investidas',
    'Concentração Geográfica Excessiva de Hubs e Aceleradoras',
    'Deficiências Estruturais na Educação STEM',
    'Descoordenação Sistêmica Entre os Agentes e Fragmentação Normativa',
    'Descumprimento de Obrigações Orçamentárias em Fundações Estaduais de Fomento à Pesquisa',
    'Desvio de Finalidade de Fundos Setoriais para Promoção da Inovação',
    'Dificuldade na Continuidade de Políticas Públicas Entre Gestões',
    'Dificuldades de Acesso e Venda a Grandes Empresas',
    'Escassez de Educação para o Empreendedorismo',
    'Escassez de Recursos Para Fomento Direto à Cultura de Inovação',
    'Estigmatização das Relações Entre Academia e Mercado',
    'Estrutura societária inadequada para startups: MEI, LTDA e S/A',
    'Incentivos escassos para negócios de impacto',
    'Incertezas Sobre a Regulação de Inteligência Artificial',
    'Instrumentos de Fomento e Crédito Desalinhados',
    'Lacunas em Letramento Digital e IA',
    'Lei de Informática e Lei do Bem Complexas, Limitadas, Complexas e Rígidas',
    'Lentidão e Insegurança Jurídica nos Processos de Transferência de Tecnologia',
    'Marco Civil da Internet em Xeque e Regulação das Plataformas Sociais',
    'Marco Legal das Startups Incompleto e Morosidade Institucional',
    'Mercado de Investimento-Anjo Subdesenvolvido e Desincentivado',
    'Modelo de Sustentabilidade Frágil dos Ambientes de Inovação',
    'Modelos Alternativos de Financiamento Ainda Incipientes Para Startups',
    'Morosidade e penalização da pessoa empreendedora no Processo de fechamento de empresas',
    'Morosidade na Obtenção de Patentes',
    'O "Custo Brasil": Complexidade Para Operar Negócios',
    'Políticas de Incentivo Insuficientes',
    'Programas de P&D&I Setoriais Incentivados Complexos e Pouco Eficazes',
    'Qualidade Relativa Incipiente dos Ambientes de Inovação',
    'Regulações Setoriais Rígidas e Descoordenadas',
    'Rigidez Regulatória Refratária à Inovação',
    'Sistema de Incentivos à Pesquisa Desalinhados',
    'Subrepresentatividade regional',
    'Tratamento Trabalhista Desproporcional Para Startups',
    'Tratamento Tributário Desproporcional Para Investidores Anjos: Desconsideração de Perdas na Apuração do Ganho de Capital',
    'Tratamento Tributário Desproporcional Para Startups',
    'Tratamento desproporcional da regulamentação da Lei de Proteção de Dados Pessoais Para Startups',
    'Vácuo Regulatório do Novos Modelos de Trabalho da Economia Digital',
]

# Mapeamento PERFEITO e DEFINITIVO
MAPEO_FINAL = {
    'ar': {
...
def limpar_e_regenerar():
    """LIMPA TUDO e REGENERA com traduções LIMPAS"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()

    for idioma in ['ar', 'ja', 'ko', 'he', 'it']:
        print(f"\nLimpando {idioma.upper()}...")

        # DELETAR TODAS as entries deste idioma
        cursor.execute("DELETE FROM fila_pesquisas WHERE idioma = ? AND status = 'pendente'", (idioma,))
        deletadas = cursor.rowcount
        print(f"  ✓ {deletadas} queries deletadas")

        # REGENERAR do zero com as tradições LIMPAS
        print(f"  Regenerando com traduções limpas...")
        mapeamento = MAPEO_FINAL.get(idioma, {})
        regeneradas = 0

        for titulo_pt, titulo_traduzido in mapeamento.items():
            # Inserir 5 cópias de cada (aproximadamente compatível com volume anterior)
            for _ in range(5):
                cursor.execute(
                    "INSERT INTO fila_pesquisas (idioma, query, status) VALUES (?, ?, 'pendente')",
                    (idioma, titulo_traduzido)
                )
                regeneradas += 1

        print(f"  ✓ {regeneradas} queries regeneradas LIMPAS")

    conn.commit()
    conn.close()

    print(f"\n✓ Limpeza e regeneração COMPLETAS!")
    print(f"✓ TODAS as queries agora contêm APENAS as traduções limpas dos 50 títulos")
