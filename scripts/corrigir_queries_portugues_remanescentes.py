...
# -*- coding: utf-8 -*-
"""
Identifica e corrige TODAS as queries que ainda contêm português em idiomas não-portugueses.
Usa pattern matching para encontrar as falhas e substituir com tradução completa.
"""
import sqlite3
import re
...
# Títulos em português para tradução
TITULOS_PT = [
...
# Mapeamento COMPLETO de TODOS os títulos para todas as línguas
MAPEO_COMPLETO = {
    'ar': {
...
def corrigir_queries():
    """Corrige queries que ainda contêm português"""
    ...
    for idioma in ['ar', 'ja', 'ko', 'he', 'it']:
        print(f"\nCorrigindo {idioma.upper()}...")

        mapeamento = MAPEO_COMPLETO.get(idioma, {})
        corrigidas_idioma = 0

        # Para cada título em português
        for titulo_pt, titulo_traduzido in mapeamento.items():
            # Atualizar TODAS as queries que contenham este título (case-insensitive, parcial)
            # Use LIKE para encontrar qualquer query que contenha o título português
            cursor.execute(
                f"UPDATE fila_pesquisas SET query = ? WHERE idioma = ? AND query LIKE ? AND status = 'pendente'",
                (titulo_traduzido, idioma, f"%{titulo_pt[:20]}%")  # Use primeiros 20 chars
            )
            corrigidas_idioma += cursor.rowcount

        print(f"  ✓ {corrigidas_idioma} queries corrigidas")
        total_corrigidas += corrigidas_idioma

    conn.commit()
    conn.close()

    print(f"\n✓ Total: {total_corrigidas} queries corrigidas")
